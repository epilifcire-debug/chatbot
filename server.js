...
let lastQR = null;
let BOT_START_TIME = Math.floor(Date.now() / 1000); // ignora msgs antigas
...
const client = new Client({
  ...
  webVersionCache: {
    type: 'remote',
    remotePath: 'https://raw.githubusercontent.com/wppconnect-team/wa-version/main/html/2.3000.1014987762-alpha.html',
  },
});
...
client.on('ready', async () => {
  ...
  BOT_START_TIME = Math.floor(Date.now() / 1000); // atualiza ao conectar
...
client.on('message', async (msg) => {
  ...
  // Ignora mensagens anteriores ao início do bot (evita spam no reconectar)
